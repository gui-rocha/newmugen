...
try:
    ...
    use = environment
    environment['MUGEN_BUILD_TESTS'] = True
    shared = SConscript('src/SConscript', variant_dir = 'build/%s' % buildType(environment['MUGEN_PLATFORM']), exports = ['use'] );
...
try:
    ...
    use = environment
    environment['MUGEN_BUILD_TESTS'] = True
    static = SConscript('src/SConscript', variant_dir = 'build/static/%s' % buildType(environment['MUGEN_PLATFORM']), exports = ['use'] );
