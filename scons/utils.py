    ...
    try:
        for backend in backends:
            if backend == 'SDL' and config.CheckSDL():
                ...
                environment['MUGEN_BACKEND'] = 'sdl'
                environment.Append(MUGEN_PLATFORM = ['sdl'])
                raise OkBackend()
            if backend == 'Allegro4' and config.CheckAllegro4():
                environment.Append(CPPDEFINES = ['USE_ALLEGRO'])
                environment['MUGEN_BACKEND'] = 'allegro4'
                environment.Append(MUGEN_PLATFORM = ['allegro4'])
                raise OkBackend()
            if backend == 'Allegro5' and config.CheckAllegro5():
                environment.Append(CPPDEFINES = ['USE_ALLEGRO5'])
                environment['MUGEN_BACKEND'] = 'allegro5'
                environment.Append(MUGEN_PLATFORM = ['allegro5'])
                raise OkBackend()
        config.Finish()
        raise NoBackend()
    except OkBackend:
